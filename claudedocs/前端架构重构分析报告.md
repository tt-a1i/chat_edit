...
```javascript
// ❌ 使用 let 而非 Vue 响应式
let quill = null        // 应该是 ref<Quill | null>
let toolbar = null      // 应该是 ref<HTMLElement | null>
let currentRange = null // 应该是 ref<Range | null>
```
...
```typescript
// 1. 未使用的变量/导入
import { unused } from 'module'  // ❌ 41 处
...
const dbLayer = {
  async getAllChats() { return db.chats.toArray() },
  async getChat(chatId) { return db.chats.get(chatId) },
  // ... 15 个方法直接操作 Dexie
}
